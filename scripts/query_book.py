...
def _effective_context(
    ...
    # Formatting overhead is small but provider tokenizers differ, so trim by
    # whole evidence chunks until the exact local estimate is within the limit.
...
def main(argv: list[str]) -> int:
    ...
    if args.retrieval_only:
        args.generate = False
    # --retrieval is intentionally accepted as an alias without changing old
    # invocations; argparse cannot express a dynamic default cleanly.
    ...
    try:
        ...
        return 1
    if not results:
        print("No indexed evidence matched. Run `ingest_book.py index <project-root>` or broaden the query.")
        return 0
