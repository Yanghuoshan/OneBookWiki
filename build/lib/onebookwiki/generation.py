...
def _refs_for(chunks: list[dict], ids: set[str], chapter: int) -> list[EvidenceRef]:
    refs: list[EvidenceRef] = []
    for index, chunk in enumerate(chunks, 1):
        evidence_id = f"C{chapter}E{index}"
        if evidence_id not in ids:
            continue
        refs.append(EvidenceRef(evidence_id=evidence_id, chunk_id=str(chunk.get("chunk_id", "")), source_path=str(chunk.get("source_path", "")), chapter=int(chunk.get("chapter", 0)), start_line=int(chunk.get("start_line", 0)), end_line=int(chunk.get("end_line", 0)), quote=""))
    return refs
...
def generate_chapters(root: Path, options: GenerationOptions | None = None, chapters: list[int] | None = None) -> CheckpointStore:
    ...
    with project_lock(root):
        for position, number in enumerate(selected_numbers, 1):
            ...
            refs = _refs_for(chunks, ids or valid_ids, number)
    ...
    for start in range(0, len(cards), max(1, options.rollup_size)):
        ...
        if not options.dry_run:
            if not store.reusable(node_id, input_hash, digest(prompt), model_hash, artifact):
                ...
                value = _parse_node_response(response, store, node_id, options)
                ids = _claim_ids(value)
                valid = {ref.evidence_id for chapter in chapters for ref in chapter.evidence}
                if ids - valid:
                    unknown = sorted(ids - valid)
                    store.mark(node_id, "failed", error=f"unknown evidence ids: {unknown}")
                    _progress(options, f"{node_id}: failed evidence validation")
                    raise GenerationError(f"rollup contains unknown evidence ids: {unknown}")
