...
def book_prompt(rollups: list[dict], chapters: list[dict], title: str, language: str = "zh-CN") -> str:
    return (
        "Produce JSON only with keys overview, core_thesis, argument_chain, themes, concepts, "
        "arguments, terminology, unresolved_questions, chapter_summaries. Claims must cite "
        "evidence_ids from the supplied artifacts; do not add external knowledge or links. "
        f"Write in {language}.\n\nBOOK TITLE: {title}\n"
        f"ROLLUPS:\n{_json(rollups)}\n\nCHAPTERS:\n{_json(chapters)}"
    )
